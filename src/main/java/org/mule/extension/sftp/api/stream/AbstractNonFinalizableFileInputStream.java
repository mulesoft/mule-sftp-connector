 ...
 * LICENSE.txt file.
 */
package org.mule.extension.sftp.api.stream;
 ...
 * created through a {@link org.mule.extension.sftp.api.stream.LazyStreamSupplier}. This allows such connection/resource to be
 * provisioned lazily. This is very useful in cases such as
 * {@link FileSystem#list(FileConnectorConfig, String, boolean, Predicate)}. Being able to only lazily establish the connections,
 * prevents the connector from opening many connections at the same time, at the risk that many of them might end up not being
...
public abstract class AbstractNonFinalizableFileInputStream extends ProxyInputStream {
  ...
  private static InputStream createLazyStream(org.mule.extension.sftp.api.stream.LazyStreamSupplier streamFactory) {
  ...
  private final org.mule.extension.sftp.api.stream.LazyStreamSupplier streamSupplier;
  private final Lock lock;
  private final AtomicBoolean closed = new AtomicBoolean(false);

  public AbstractNonFinalizableFileInputStream(org.mule.extension.sftp.api.stream.LazyStreamSupplier streamSupplier,
                                               PathLock lock) {
